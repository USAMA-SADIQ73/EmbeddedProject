...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        // msg_id = esp_mqtt_client_publish(client, "Sensor/qos1", "data_3", 0, 1, 1);
        ...
        msg_id = esp_mqtt_client_subscribe(client, "Actuator/Fan", 1);
        ...
        esp_mqtt5_client_set_user_property(&subscribe_property.user_property, user_property_arr, USE_PROPERTY_ARR_SIZE);
        esp_mqtt5_client_set_subscribe_property(client, &subscribe_property);
        msg_id = esp_mqtt_client_subscribe(client, "Actuator/LED", 1);
        esp_mqtt5_client_delete_user_property(subscribe_property.user_property);
        subscribe_property.user_property = NULL;
        ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
    ...
    mqtt5_app_start();
      ...
      // integer to string function
        ...
        while (1)
        {
             ...
             sprintf(Dtemp, "%d", temp);
             msg_id = esp_mqtt_client_publish(client, "/Sensor/Temp", Dtemp, 0, 0, 0);
             ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
                vTaskDelay(1000 / portTICK_PERIOD_MS);

             sprintf(Dhumid, "%d", humid);
             msg_id = esp_mqtt_client_publish(client, "/Sensor/Humid", Dtemp, 0, 0, 0);
             ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
